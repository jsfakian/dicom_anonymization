...
### Handling of Dose Reports

The tool supports anonymization of DICOM-based Radiation Dose Structured Reports (RDSR) and will process these files automatically when they are included in the dataset. However, the tool does not process text-based dose exports, such as dose.txt or rdsr.txt files generated by scanners, PACS, or dose monitoring systems. Text-based dose reports are not standardized and cannot be reliably anonymized; as such, they should handled separately according to local privacy procedures.
